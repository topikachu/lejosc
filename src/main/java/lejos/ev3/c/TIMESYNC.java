package lejos.ev3.c;
import com.sun.jna.Pointer;
import com.sun.jna.Structure;
import java.util.Arrays;
import java.util.List;
/**
 * <i>native declaration : lms2012.h</i><br>
 * This file was autogenerated by <a href="http://jnaerator.googlecode.com/">JNAerator</a>,<br>
 * a tool written by <a href="http://ochafik.com/">Olivier Chafik</a> that <a href="http://code.google.com/p/jnaerator/wiki/CreditsAndLicense">uses a few opensource projects.</a>.<br>
 * For help, please visit <a href="http://nativelibs4java.googlecode.com/">NativeLibs4Java</a> , <a href="http://rococoa.dev.java.net/">Rococoa</a>, or <a href="http://jna.dev.java.net/">JNA</a>.
 */
public class TIMESYNC extends Structure {
	/** C type : DATA8 */
	public byte Cmd;
	public byte getCmd() {
		return Cmd;
	}
	public void setCmd(byte Cmd) {
		this.Cmd = Cmd;
	}
	/** C type : DATA8 */
	public byte Nos;
	public byte getNos() {
		return Nos;
	}
	public void setNos(byte Nos) {
		this.Nos = Nos;
	}
	/** C type : DATA8 */
	public byte Speed;
	public byte getSpeed() {
		return Speed;
	}
	public void setSpeed(byte Speed) {
		this.Speed = Speed;
	}
	/** C type : DATA16 */
	public short Turn;
	public short getTurn() {
		return Turn;
	}
	public void setTurn(short Turn) {
		this.Turn = Turn;
	}
	/** C type : DATA32 */
	public int Time;
	public int getTime() {
		return Time;
	}
	public void setTime(int Time) {
		this.Time = Time;
	}
	/** C type : DATA8 */
	public byte Brake;
	public byte getBrake() {
		return Brake;
	}
	public void setBrake(byte Brake) {
		this.Brake = Brake;
	}
	public TIMESYNC() {
		super();
	}
	public TIMESYNC(Pointer address) {
		super(address);
	}
	protected List<? > getFieldOrder() {
		return Arrays.asList("Cmd", "Nos", "Speed", "Turn", "Time", "Brake");
	}
	/**
	 * @param Cmd C type : DATA8<br>
	 * @param Nos C type : DATA8<br>
	 * @param Speed C type : DATA8<br>
	 * @param Turn C type : DATA16<br>
	 * @param Time C type : DATA32<br>
	 * @param Brake C type : DATA8
	 */
	public TIMESYNC(byte Cmd, byte Nos, byte Speed, short Turn, int Time, byte Brake) {
		super();
		this.Cmd = Cmd;
		this.Nos = Nos;
		this.Speed = Speed;
		this.Turn = Turn;
		this.Time = Time;
		this.Brake = Brake;
	}
	public static class ByReference extends TIMESYNC implements Structure.ByReference {
		
	};
	public static class ByValue extends TIMESYNC implements Structure.ByValue {
		
	};
}
